import { ViaKeyboard } from "./vialKeyboad";

export const QuantumSettingDefinition: {
  label: string;
  content: {
    type: string;
    label: string;
    bit?: number;
    content: (string | number)[];
    options?: (string | number)[] | (string | number)[][];
  }[];
}[] = [
  {
    label: "Magic",
    content: [
      {
        type: "checkbox-list",
        label: "Magic",
        content: ["id-magic", 21, 2],
        options: [
          ["Swap Caps Lock and Left Control", 0],
          ["Treat Caps Lock as Control", 1],
          ["Swap Left Alt and GUI", 2],
          ["Swap Right Alt and GUI", 3],
          ["Disable the GUI keys", 4],
          ["Swap ` and Escape", 5],
          ["Swap \\ and Backspace", 6],
          ["Enable N-key rollover", 7],
          ["Swap Left Control and GUI", 8],
          ["Swap Right Control and GUI", 9],
        ],
      },
    ],
  },
  {
    label: "Grave Escape",
    content: [
      {
        type: "checkbox-list",
        label: "Grave Escape Override",
        content: ["id-grave-escape", 1, 1],
        options: [
          ["Always send Escape if Alt is pressed", 0],
          ["Always send Escape if Control is pressed", 1],
          ["Always send Escape if GUI is pressed", 2],
          ["Always send Escape if Shift is pressed", 3],
        ],
      },
    ],
  },

  {
    label: "Tap-Hold",
    content: [
      {
        type: "number",
        label: "Tapping Term",
        content: ["id-tapping-term", 7, 2],
        options: [0, 10000],
      },
      {
        type: "boolean",
        label: "Permissive Hold",
        content: ["id-permissive-hold", 22, 1],
      },
      {
        type: "boolean",
        label: "Hold On Other Key Press",
        content: ["id-hold-on-other-key-press", 23, 1],
      },
      {
        type: "boolean",
        label: "Retro Tapping",
        content: ["id-retro-tapping", 24, 1],
      },
      {
        type: "number",
        label: "Quick Tap Term",
        content: ["id-quick-tap-term", 25, 2],
        options: [0, 10000],
      },
      {
        type: "number",
        label: "Tap Code Delay",
        content: ["id-tap-code-delay", 18, 2],
        options: [0, 1000],
      },
      {
        type: "number",
        label: "Tap Hold Caps Delay",
        content: ["id-tap-hold-caps-delay", 19, 2],
        options: [0, 1000],
      },
      {
        type: "number",
        label: "Tapping Toggle",
        content: ["id-tapping-toggle", 20, 1],
        options: [0, 100],
      },
      {
        type: "boolean",
        label: "Chordal Hold",
        content: ["id-chordal-hold", 26, 1],
      },
      {
        type: "number",
        label: "Flow Tap",
        content: ["id-flow-tap", 27, 2],
        options: [0, 10000],
      },
    ],
  },
  {
    label: "Auto Shift",
    content: [
      {
        type: "bit-checkbox",
        label: "Enable",
        bit: 0,
        content: ["id-auto-shift", 3, 1],
      },
      {
        type: "bit-checkbox",
        label: "Enable for modifiers",
        bit: 1,
        content: ["id-auto-shift", 3, 1],
      },
      {
        type: "number",
        label: "Timeout",
        content: ["id-auto-shift-timeout", 4, 2],
        options: [0, 1000],
      },
      {
        type: "bit-checkbox",
        label: "Do not Auto Shift special keys",
        bit: 2,
        content: ["id-auto-shift", 3, 1],
      },
      {
        type: "bit-checkbox",
        label: "Do not Auto Shift numeric keys",
        bit: 3,
        content: ["id-auto-shift", 3, 1],
      },
      {
        type: "bit-checkbox",
        label: "Do not Auto Shift alpha characters",
        bit: 4,
        content: ["id-auto-shift", 3, 1],
      },
      {
        type: "bit-checkbox",
        label: "Enable keyrepeat",
        bit: 5,
        content: ["id-auto-shift", 3, 1],
      },
      {
        type: "bit-checkbox",
        label: "Disable keyrepeat when timeout is exceeded",
        bit: 6,
        content: ["id-auto-shift", 3, 1],
      },
    ],
  },

  {
    label: "Combo",
    content: [
      {
        type: "number",
        label: "Time out period for combos",
        content: ["id-combo-term", 2, 2],
        options: [0, 10000],
      },
    ],
  },
  {
    label: "One Shot Keys",
    content: [
      {
        type: "number",
        label: "Tapping this number of times holds the key until tapped once again",
        content: ["id-osk-tap-toggle", 5, 1],
        options: [0, 50],
      },
      {
        type: "number",
        label: "Time (in ms) before the one shot key is released",
        content: ["id-osk-tap-timeout", 6, 2],
        options: [0, 60000],
      },
    ],
  },
  {
    label: "Mouse Keys",
    content: [
      {
        type: "number",
        label: "Delay between pressing a movement key and cursor movement",
        content: ["id-mousekey-delay", 9, 2],
        options: [0, 1000],
      },
      {
        type: "number",
        label: "Time between cursor movements in milliseconds",
        content: ["id-mousekey-interval", 10, 2],
        options: [0, 1000],
      },
      {
        type: "number",
        label: "Step size",
        content: ["id-mousekey-move-delta", 11, 2],
        options: [0, 255],
      },
      {
        type: "number",
        label: "Maximum cursor speed at which acceleration stops",
        content: ["id-mousekey-max-speed", 12, 2],
        options: [0, 255],
      },
      {
        type: "number",
        label: "Time until maximum cursor speed is reached",
        content: ["id-mousekey-time-to-max", 13, 2],
        options: [0, 1000],
      },
      {
        type: "number",
        label: "Delay between pressing a wheel key and wheel movement",
        content: ["id-mousekey-wheel-delay", 14, 2],
        options: [0, 1000],
      },
      {
        type: "number",
        label: "Time between wheel movements",
        content: ["id-mousekey-wheel-interval", 15, 2],
        options: [0, 1000],
      },
      {
        type: "number",
        label: "Maximum number of scroll steps per scroll action",
        content: ["id-mousekey-wheel-max-speed", 16, 2],
        options: [0, 255],
      },
      {
        type: "number",
        label: "Time until maximum scroll speed is reached",
        content: ["id-mousekey-wheel-time-to-max", 17, 2],
        options: [0, 1000],
      },
    ],
  },
];

export async function QuantumSettingsReadAll(via: ViaKeyboard): Promise<{ [id: string]: number }> {
  const ids = QuantumSettingDefinition.flatMap((set) =>
    set.content.map((setting) => setting.content),
  );

  return Object.entries(await via.GetQuantumSettingsValue(ids.map((id) => id[1] as number))).reduce(
    (acc, v) => {
      return {
        ...acc,
        [v[0]]:
          v[1] & ((1 << ((ids.find((k) => k[1].toString() === v[0])?.[2] as number) * 8)) - 1),
      };
    },
    {} as { [id: string]: number },
  );
}
